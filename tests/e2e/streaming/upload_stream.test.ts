...
describe("HTTP/3 Upload Streaming", () => {
  ...
  describe("/upload/stream endpoint", () => {
    it("handles small uploads and returns correct stats", async () => {
      ...
      const response = await post(`https://127.0.0.1:${server.port}/upload/stream`, testData, {
        headers: {
    ...
    it("handles medium-sized uploads (10MB)", async () => {
      await withTempDir(async () => {
        ...
        console.log(`10MB upload: ${json.throughput_mbps.toFixed(2)} Mbps, ${json.elapsed_ms}ms`);
      });
    });
