...
describe("H3 DATAGRAM Tests", () => {
  test("request-associated H3 dgram echo", async () => {
    ...
    try {
      ...
      expect(new TextDecoder().decode(response.body)).toContain("flow_id for this request is the stream_id");
      
      // Test H3 DATAGRAM echo functionality using quiche-client helper
      const { quicheClient } = await import("@helpers/quicheClient");
      
      await retryWithBackoff(async () => {
        const res = await quicheClient(`https://127.0.0.1:${server.port}/h3dgram/echo`, {
          dumpJson: false,  // We want raw output for DATAGRAM logs
          dgramProto: "oneway",
          dgramCount: 3,
          idleTimeout: 5000,  // 5 second idle timeout to allow echo responses
        });
  ...
  test("unknown flow_id handling", async () => {
  ...
  test("varint encoding/decoding boundary cases", async () => {
