...
describe("HTTP/3 Routing", () => {
  ...
  describe("Wildcard routes", () => {
    ...
    it("GET /files/* captures query parameters", async () => {
      const response = await get(`https://127.0.0.1:${server.port}/files/data.json?format=pretty`);
  ...
  describe("Edge cases", () => {
    ...
    it("Handles URL-encoded parameters", async () => {
      ...
      const response = await get(`https://127.0.0.1:${server.port}/api/users/${encodedParam}`);
